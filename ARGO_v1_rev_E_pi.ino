...
//
        ...
        /////////////////
            ...
            // Limit Switch Inputs
            const int limitSwitchOpenPin        = 12;
            const int limitSwitchClosedPin      = 13;
...
//
        ...
        float Motor_Thermistor_Temperature()
        {
            ...
            Tc = (1.0 / ((1.0 / T0_therm) + (1.0/B_val) * log(R_meas / R0_therm)) );
            Tk = Tc + 273.15;
            Tf = (Tc * 9.0)/ 5.0 + 32.0; 
...
//
        ...
        void loop()   // the loop function runs over and over again forever   
        {
            ...
            while (Serial.available() > 0)   // Constantly checks to see if anything has been sent over the USB Connection and if it needs to be processed
            {
                ...
                Serial.println(F("\t4.) Check IR Sensor Thermistor Temperature"));
                Serial.println(F("\t5.) Check Motor Thermistor Temperature"));
                Serial.println(F("\t6.) "));
                Serial.println(F("\t7.) "));                
                Serial.println(F("\t8.) IR Camera Test"));
                Serial.println(F("\t9.) Motor-Synced LED Test\n\n"));
                ...
                // Main Menu
                    ...
                    if(menuChoice >= 48 && menuChoice <= 57) // if a valid menuChoice ASCII 0-9 was selected, see ASCII table for reference
                    {
                    ...
                    switch(menuChoice)
                    {
                        ...
                        case '2': close_Shutter();
                                  break;
                        case '3': take_IR_Image();
                                  break;
                        case '4': IR_Thermistor_Temperature();
                                  break;
                        case '5': Motor_Thermistor_Temperature();
                                  break;
                        case '8': IR_Camera_Test();
                                  break;          
                        case '9': motor_LED_Test();
                                  break;
